...
function certDir(domain) { return path.join(LE_LIVE, domain); }
function hasCert(domain) { return fs.existsSync(path.join(certDir(domain), 'fullchain.pem')); }
...
function buildConfig(row) {
  ...
  // No cert yet: serve over HTTP only and keep the ACME path reachable so a
  // cert can be issued. Avoids referencing a cert file that doesn't exist
  // (which would fail `nginx -t`). Flips to HTTPS once the cert is in place.
  if (!hasCert(row.domain)) {
    return `server {
    listen 80; listen [::]:80;
    server_name ${names};
